...
class Day_10 {
  ...
  func dfs(map: inout [[Int]], position: Position, recursiveCount: inout Int, part2: Bool = false) {
    ...
    if inBounds(position: east) {
      if map[east.row][east.column] == currentIncline + 1 {
        dfs(map: &map, position: east, recursiveCount: &recursiveCount, part2: part2)
      }
    }
    
    let south = position + .SOUTH
    if inBounds(position: south) {
      if map[south.row][south.column] == currentIncline + 1 {
        dfs(map: &map, position: south, recursiveCount: &recursiveCount, part2: part2)
      }
    }
    
    let west = position + .WEST
    if inBounds(position: west) {
      if map[west.row][west.column] == currentIncline + 1 {
        dfs(map: &map, position: west, recursiveCount: &recursiveCount, part2: part2)
      }
    }
    
    let north = position + .NORTH
    if inBounds(position: north) {
      if map[north.row][north.column] == currentIncline + 1 {
        dfs(map: &map, position: north, recursiveCount: &recursiveCount, part2: part2)
      }
    }
  }
